  ...
  class GcloudStorage extends IStorage {
    ...
    constructor(options, name) {
      ...
      if (typeof this[config].credentials === 'string' && this[config].credentials) {
        let credPath = path.normalize(thorin.root + '/' + this[config].credentials);
        try {
    ...
    save(fileObj) {
      return new Promise((resolve, reject) => {
        ...
        sFileObj.save(fileObj.getStream(), fOpt, function (err, res) {
          if (err) {
    ...
    download(fileUrl, opt) {
      ...
      return new Promise((resolve, reject) => {
        ...
        function onDone(e) {
          ...
          logger.debug(e);
          reject(thorin.error('UPLOAD.DOWNLOAD_FAILED', 'Could not download file', e));
        }
