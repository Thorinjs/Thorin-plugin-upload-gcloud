...
module.exports = function (thorin, opt, IStorage) {
  ...
  class GcloudStorage extends IStorage {
    ...
    constructor(options, name) {
      ...
      if (this[config].workloadIdentity && typeof this[config].workloadIdentity === 'object') {
        if (!this[config].bucket) {
          logger.warn('GCloud Storage [workload identity]: missing bucket in configuration');
          return;
        }
        if (!this[config].workloadIdentity.serviceAccountEmail) {
          logger.warn('GCloud Storage [workload identity]: missing serviceAccountEmail — signed URLs will not work');
        }
        const storageOpts = {};
        if (this[config].workloadIdentity.projectId) storageOpts.projectId = this[config].workloadIdentity.projectId;
        if (this[config].workloadIdentity.serviceAccountEmail) storageOpts.serviceAccountEmail = this[config].workloadIdentity.serviceAccountEmail;
        this[sdk] = new Storage(storageOpts);
      } else {
    ...
    save(fileObj) {
      return new Promise((resolve, reject) => {
        ...
        if (fOpt.predefinedAcl === 'public' || fOpt.predefinedAcl === 'public-read') {
          fOpt.predefinedAcl = 'publicRead';
        }
        ...
        if (typeof streamObj === 'string' || streamObj instanceof Buffer) {
          if (fOpt.predefinedAcl === 'publicRead') {
            fOpt.public = true;
          }
          sFileObj.save(streamObj, fOpt, onDone);
        } else {
          let writeOpt = {
            //resumable: fOpt.resumable,
            metadata: {
              contentType: fOpt.contentType
            },
            predefinedAcl: fOpt.predefinedAcl
          };
          if (writeOpt.predefinedAcl === 'publicRead') {
            writeOpt.public = true;
          }
          if (fOpt.contentDisposition) {
            writeOpt.metadata.contentDisposition = fOpt.contentDisposition;
          }
          if (fOpt.cacheControl) {
            writeOpt.metadata.cacheControl = fOpt.cacheControl;
          }
          let remoteWrite = sFileObj.createWriteStream(writeOpt);
